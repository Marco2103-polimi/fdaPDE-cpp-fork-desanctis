...
template <typename VariationalSolver> class MSRPDE {
   ...
   public:
    ...
    template <typename GeoFrame> void analyze_data(const std::string& formula, const GeoFrame& gf) {
        return analyze_data(formula, gf, initial_weights_());
    }
    ...
    auto fit(Args&&... args) {
        ...
        // initialization (nota M: eseguito due volte la prima volta che viene chiamato il fit, ma necessario se viene chiamato più volte il fit)      
        initial_weights_(); 
        solver_.update_response_and_weights(y, sparse_mat_weights_);   // restore solver state
        ...
        while (n_iter_ < max_iter_ && std::abs(Jnew - Jold) > tol_) {
            ...
            compute_sigma_sq_hat_();  // note: default value is false => metodo Melchionda (calcolo senza edf nelle fpirls iterations)
            build_LTL_();
            compute_C_();
    ...
    vector_t random_effects() const {   // M: solo la parte dei random effects del fit 
        vector_t random_effects = vector_t::Zero(n_obs_);
        for(int i=0; i<n_groups_; ++i){
            random_effects(loc_to_glob_map_[i]) += Z_(i) * b_hat_[i];
        }
    ...
    // M nota: dopo group_ids_ farà parte del geoframe e questo setter non servirà più
    void set_ids_groups(Eigen::Matrix<unsigned int, Dynamic, 1> Rgroup_ids){  // Rgroup_ids: vector storing the id (integer) that identifies each observation to a specific group
        
        std::cout << "Rgroup_ids.size() = " << Rgroup_ids.size() << std::endl;
        ...
        group_ids_ = Rgroup_ids - Eigen::Matrix<unsigned int, Dynamic, 1>::Ones(Rgroup_ids.size()); // ATT: aggiunto -1 perchè come input do Rgroup_ids che parte a contare da 1 (anche Melchionda lo fa, ma nel wrapper)
    ...
    private:
        ...
        // ATT in gsr e qsr pW_ è un vettore, qui è un vettore di matrici 
        ...
        Eigen::Matrix<matrix_t, Dynamic, 1> Z_;  // vectors of blocks storing the random effect matrices Z_i, i=1,...,n_groups_ (by groups)
    ...
    private:
        ...
        void Delta_init_(){
            ...
            for(int i=0; i < n_groups_; ++i){
                ...
                Z_(i) = matrix_indexing_(Z(), loc_to_glob_map_[i]);  
        ...
        void resize_matrices_(){
            ...
            Z_.resize(n_groups_);
            for(int i=0; i<n_groups_; ++i){
                Z_(i).resize(group_sizes_[i], n_random_covs_); 
            }
        ...
        sparse_matrix_t initial_weights_() {
            resize_matrices_(); // allocate memory
            std::cout << "sparse_mat_weights_.rows()=" << sparse_mat_weights_.rows() << std::endl;
            std::cout << "sparse_mat_weights_.cols()=" << sparse_mat_weights_.cols() << std::endl;
            Delta_init_();      // smart initialization of Delta_
            std::cout << "Delta_.rows()=" << Delta_.rows() << std::endl;
            std::cout << "Delta_.cols()=" << Delta_.cols() << std::endl;
            update_ZtildeTZtilde_(); 
            update_pW_(); 
            std::cout << "pW_.size()=" << pW_.size() << std::endl;
            std::cout << "pW_(1).rows()=" << pW_(1).rows() << std::endl;
            std::cout << "pW_(1).cols()=" << pW_(1).cols() << std::endl;
            std::cout << "sparse_mat_weights_.rows()=" << sparse_mat_weights_.rows() << std::endl;
            std::cout << "sparse_mat_weights_.cols()=" << sparse_mat_weights_.cols() << std::endl;
        ...
        matrix_t Z() const { 
            return matrix_t::Ones(n_obs_, 1);   // M ATT: temporary!!!  
            // return solver_.random_design_matrix();
        } 
        ...
        void compute_sigma_sq_hat_(bool edf_flag=false) {
