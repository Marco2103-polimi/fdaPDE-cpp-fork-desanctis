...
//     const bool edf_in_sigma_computation = true; // true to use edf in sigma computation
//     const bool likelihood_data_loss_flag = true; // false for FPIRLS loss; true per likelihood-type loss
//     const bool true_fpirls_normalize = true; // ATT: you have to modify fpirls-h accordingly!
...
//     const bool normalized_loss_flag = false;  // useless
...
//     if(est_type == "mixed"){
//         if(edf_in_sigma_computation && likelihood_data_loss_flag && !true_fpirls_normalize){
//             solutions_path = solutions_path + "/sigma_edf";
//         }
//         if(edf_in_sigma_computation && !likelihood_data_loss_flag && true_fpirls_normalize){
//             solutions_path = solutions_path + "/sigma_edf_2";
//         }
//         if(edf_in_sigma_computation && likelihood_data_loss_flag && true_fpirls_normalize){
//             solutions_path = solutions_path + "/sigma_edf_3";
//         }       
//     }
...
//         model.set_temporal_locations(time_locs);
//         model.set_normalize_loss(normalized_loss_flag);
//         model.set_sigma_with_edf(edf_in_sigma_computation);
//         model.set_likelihood_data_loss(likelihood_data_loss_flag);
...
//         if(edf_in_sigma_computation && likelihood_data_loss_flag && !true_fpirls_normalize){
//             solutions_path = solutions_path + "/sigma_edf";
//             solutions_path_gcv = solutions_path_gcv + "/sigma_edf";
//         }
//         if(edf_in_sigma_computation && !likelihood_data_loss_flag && true_fpirls_normalize){
//             solutions_path = solutions_path + "/sigma_edf_2";
//             solutions_path_gcv = solutions_path_gcv + "/sigma_edf_2";
//         }
//         if(edf_in_sigma_computation && likelihood_data_loss_flag && true_fpirls_normalize){
//             solutions_path = solutions_path + "/sigma_edf_3";
//             solutions_path_gcv = solutions_path_gcv + "/sigma_edf_3";
//         }       
//     }

//     std::cout << "solution path: " << solutions_path << std::endl; 
//     std::cout << "solution path GCV: " << solutions_path_gcv << std::endl;
...
// k-fold errors 
TEST(case_study_mstrpde_Kfold, NO2) {
    ...
    unsigned int num_folds = 10;        // numero di fold per il CV error
    std::vector<std::string> kfold_versions = {"CAR", "WS"};  // "CAR" (Completely At Random); "WS" (Whole Station)
    ...
    std::string pde_type = "tr";  // ""  "tr" 
    const std::string u_string = "1e-1"; 
    std::string CV_type = "5-folds"; // questo NON e' il numero di folds del CV error, ma il numero di fold usato per selezionare i lambdas!!
    ...
    std::string sigma_edf_string; 
    if(edf_in_sigma_computation && likelihood_data_loss_flag && !true_fpirls_normalize){
        sigma_edf_string = "/sigma_edf"; 
    }
    if(edf_in_sigma_computation && !likelihood_data_loss_flag && true_fpirls_normalize){
        sigma_edf_string = "/sigma_edf_2"; 
    }
    if(edf_in_sigma_computation && likelihood_data_loss_flag && true_fpirls_normalize){
        sigma_edf_string = "/sigma_edf_3"; 
    }       


    if(est_type == "mixed"){
        solutions_path = solutions_path + sigma_edf_string;
        solutions_path_gcv = solutions_path_gcv + sigma_edf_string;
    }

    std::cout << "solution path: " << solutions_path << std::endl << std::endl; 
    std::cout << "solution path GCV: " << solutions_path_gcv << std::endl << std::endl; 
    ...
    for(std::string kfold_version : kfold_versions){

        std::string kfold_path = path + "/" + results_str + "/" + std::to_string(num_folds) + "-fold_comparison" + sigma_edf_string + "/" + kfold_version;
        std::cout << "kfold_path path: " << kfold_path << std::endl << std::endl; 

        std::cout << "-----------------------------k-FOLD " + kfold_version + " STARTS------------------------" << std::endl; 

        for(int fold = 1; fold <= num_folds; ++fold){

            std::cout << "-----------FOLD #" << fold << "-----------" << std::endl;

            y = read_csv<double>(kfold_path + "/fold_" + std::to_string(fold) + "/y_train.csv");
            BlockFrame<double, int> df;
            df.stack(OBSERVATIONS_BLK, y);
            if(model_type_root == "param")
                df.insert(DESIGN_MATRIX_BLK, X);
            ...
            if(est_type == "mean" || est_type == "mean_dummies"){
                ...
                // std::cout << "setting lambda_D = " << lambda_D << std::endl;
                // std::cout << "setting lambda_T = " << lambda_T << std::endl;
                ...
                DMatrix<double> computedF = model.f();
                const static Eigen::IOFormat CSVFormatf(Eigen::FullPrecision, Eigen::DontAlignCols, ", ", "\n");
                std::ofstream filef(kfold_path + "/fold_" + std::to_string(fold) + "/f_strpde.csv");
                if(filef.is_open()){
                    filef << computedF.format(CSVFormatf);
                    filef.close();
                }

                // ....non salvo fn tanto avrebbe zeri nelle righe missing...

                if(model_type_root == "param"){
                    DMatrix<double> computedBeta = model.beta(); 
                    const static Eigen::IOFormat CSVFormatBeta(Eigen::FullPrecision, Eigen::DontAlignCols, ", ", "\n");
                    std::ofstream filebeta(kfold_path + "/fold_" + std::to_string(fold) + "/beta_strpde.csv");
                    if(filebeta.is_open()){
                        filebeta << computedBeta.format(CSVFormatBeta);
                        filebeta.close();
                    }
            ...
            if(est_type == "mixed"){
                ...
                // std::cout << "computedF max = " << (computedF.rowwise().lpNorm<1>()).maxCoeff() << std::endl; 
                const static Eigen::IOFormat CSVFormatf(Eigen::FullPrecision, Eigen::DontAlignCols, ", ", "\n");
                std::ofstream filef(kfold_path + "/fold_" + std::to_string(fold) + "/f_mstrpde.csv");
                if(filef.is_open()){
                    filef << computedF.format(CSVFormatf);
                    filef.close();
                }
                ...
                if(model_type_root == "param"){
                    DMatrix<double> computedBeta = model.beta(); 
                    const static Eigen::IOFormat CSVFormatBeta(Eigen::FullPrecision, Eigen::DontAlignCols, ", ", "\n");
                    std::ofstream filebeta(kfold_path + "/fold_" + std::to_string(fold) + "/beta_mstrpde.csv");
                    if(filebeta.is_open()){
                        filebeta << computedBeta.format(CSVFormatBeta);
                        filebeta.close();
                    }
                ...
                const static Eigen::IOFormat CSVFormatb(Eigen::FullPrecision, Eigen::DontAlignCols, ", ", "\n");
                std::ofstream fileb(kfold_path + "/fold_" + std::to_string(fold) + "/b_mstrpde.csv");
                if(fileb.is_open()){
                    fileb << computed_b.format(CSVFormatb);
                    fileb.close();
                }
                ...
                DMatrix<double> computedsigma_b_hat = model.Sigma_b();
                const static Eigen::IOFormat CSVFormatsigma_b_hat(Eigen::FullPrecision, Eigen::DontAlignCols, ", ", "\n");
                std::ofstream filesigma_b_hat(kfold_path + "/fold_" + std::to_string(fold) + "/Sigma_b_hat.csv");
                if(filesigma_b_hat.is_open()){
                    filesigma_b_hat << std::setprecision(16) << computedsigma_b_hat.format(CSVFormatsigma_b_hat);
                    filesigma_b_hat.close();
                }
